...
The valence of "Conflicted" means uncertain whether the use is happy or sad. The valence in the data set is chosen based on my personal perception. In other words, the data is very biased. 
...
### Handcrafted data
I manually ran through all the episodes to create a data frame. This was necessary for contrasting the Spotify features with the drama's feel. The data contains mostly facts with one exception : valence. This valence is based on my own judgement of the scenes. This may vary from person to person. The perceived valence from the scenes is an essential metric for finding correlations between employment of tracks and the emotional perception.
...
The energy score in Spotify signifies the perceptual measure of intensity and activity of a track. The energy feature suites really well with my own valence feature. This valence is the perception of the track in the drama.
...
### Take a peek in the chroma

Let's first take a look at how the tracks are structured and which section of the track is frequently used in the drama. This is done by using a chromagram, which is great way to visualize the track's pitches and find differences in sections. 
...
#### Chromo analysis
The parts are divided by the green vertical lines. The red line represents the average time of the "Tell me it's not a dream" being played in the drama. In general the track is being played from the intro to the average time.

In contrast to "My reason of my smiles" is this track's intro not mention worthy. It has no significant use, than setting up the track. The centre of attention is steered into the chorus and verses. 
...
#### Chromo analysis
The parts are divided by the green vertical lines. The red line represents the average time of the "Hold me back" being played in the drama. In general the track is being played from the intro to the average time.

It's observable that the scale used is C, B, A, A#, F,D# and D, because the pitch magnitudes are concentrated on horizontal axis. The spacing in the pitches is very similar to the piano part in "Tell me it's not a dream" and in fact the track is mainly accompanied by the piano. This track's chromagram has no obvious patterns, the magnitudes are all over the place. Hinting towards a high tempo track. 
...
#### Chromo analysis
The parts are divided by the green vertical lines. The red line represents the average time of the "Love you with all my heart" being played in the drama. In general the track is being played from the intro to the average time.

It's observable that the scale used is B, A, G#, F#, E, D# and C#, because the pitch magnitudes are concentrated on horizontal axis.

This track's chromagram also has no obvious patterns, the magnitude and use of the pitches does not emit any logical story.

#### Usages
Verse 1 is used in all occurrences and the first snippet of chorus 1 is added. The addition of the chorus is for fitting the scene's duration.
...
isaac_chroma |>
  ...
  geom_vline(xintercept = 60, color = "green", size = 2) +
  geom_vline(xintercept = 112, color = "green", size = 2) +
  geom_vline(xintercept = 160, color = "green", size = 2) +
  geom_vline(xintercept = 240, color = "green", size = 2) +
  
  annotate('text', x = 35, y = 6.5, label = '(Verse 1)', size=7, fontface="bold", color="orange", alpha=0.5) +
  annotate('text', x = 85, y = 6.5, label = '(Chorus 1)', size=7, fontface="bold", color="orange", alpha=0.5) +
  annotate('text', x = 140, y = 6.5, label = '(Verse 2)', size=7, fontface="bold", color="orange", alpha=0.5) +
  annotate('text', x = 200, y = 6.5, label = '(Chorus 2)', size=7, fontface="bold", color="orange", alpha=0.5)

```

#### Chromo analysis
The parts are divided by the green vertical lines. The red line represents the average time of the "Fallin'" being played in the drama. In general the track is being played from the intro to the average time.


##### Scale
It's observable that the scale used is C, A#, G#, F#, F, D# and C#, because the pitch magnitudes are concentrated on horizontal axis.


##### Low magnitude areas
The track has the most low magnitude areas. The tracks audio can be considered having the sharpest timbre. Containing fewer undertones and using instruments with a clear sound, such as the piano. 


##### Blocks
The singer can be heard to hold a high pitch for a longer time. The blocks can be interpreted as holding the pitch.

#### Lowest energy score
Judging by the magnitudes the track. The track doesn't strike as having the lowest energy score from Spotify. Maybe the amount of dark areas are what makes Spotify rank a track with a lower energy score. This also explains why "Tell me it's note a dream" has a low energy score, since it has the second most dark areas. Followed by "Love you with all my heart as third.

#### Usages
In 1 instance has the full track been used. Despite being used fully, doesn't imply anything structurally. The reason for the long use : The wife found out about the divorcement papers, while the couple were happily in Germany mending their love once more. The scene was meant to showcase the severity of breaking their trust.

Also the longer average time is largely effected by the full use instance. The average time without the full use is 70 seconds. This concludes that verse 1 and chorus 1 are mainly used.

## Row {style="min-height:200px"}

### Summarisation of the Chroma's

- The general structure of the tracks is I,V,C,V,C,O.
- The darker areas with low magnitude are highly correlated with the Spotify's a lower energy score
- The difference in timbre is visible in the chroma's. Hinting that timbre has an effect on valence en energy.
- Marked the segments which are used in the scenes. Advantaguous for filtering unneeded segments.
- Able to discern the scales used. Despite not very informative now.
...
# Timbre {data-navmenu=Analysis data-orientation=rows}

## Row {style="min-height:200px"} 

### Understand the timbre in tracks
Since we know which segments are primarily used in the drama, we can now look at timbre features in these segments. To find emerging relationship between the timbre in the tracks and the emotional scenes.
...
#### DTW insights
The deployment of DTW is to find similarities in segments. There are 2 possible scenario's : 

 - If these similarities occur in tracks used for the same purpose, then the timbre may be the causal factor.
 - When the tracks have no similarities, then the timbre of a track is unique. 
 
To my surprise there actually exist a similarity between 2 tracks : "Fallin'" and  "Love you with all my heart". In the DTW plot on the right can a continuous diagonal line been seen. This diagonal line represents the exact similarity of timbre features in that area. This similarity explains the similar employment of the tracks in the drama. Conveying a complex mixture of happiness and sadness.

#### New categorisation 
The tracks are both used for all categories of valence and can be considered all-round. With this new insight the tracks can be categorized:

- Happy : "The reason of my smiles" and "Tell me it's a dream".
- Conflicted : "Fallin'" and  "Love you with all my heart".
- Sad : "Hold me back"


### DTW of "Fallin'" &  "Love you with all my heart"

```{r, echo=FALSE, fig.width=5, fig.height=4}
compmus_long_distance(
  short_isaac |> mutate(timbre = map(timbre, compmus_normalise, "chebyshev")),
  short_crush |> mutate(timbre = map(timbre, compmus_normalise, "chebyshev")),
  feature = timbre,
  method = "euclidean"
) |>
  ggplot(
  ...
  geom_tile() +
  coord_equal() +
  labs(x = "Fallin'", y = "Love you with all my heart") +
  theme_minimal() +
...
By comparing the valence of the track's employment in the drama and the Spotify features on the tracks, is it possible to find correlations between the concepts. 

## "The reason of my smiles" 
The perceived usage of "The reason of my smiles" has a positive correlation with the Spotify features. This correlation is supported by high valence score given by Spotify and largely use in happy scenes. 

## "Tell me it's not a dream"
The perceived usage of "Tell me it's not a dream" has a negative correlation with the Spotify features. Spotify sees the track as unhappiest track. While the track is clearly used for joyful moments. 

## "Hold me back"
The perceived usage of"Hold me back" has a positive correlation with the Spotify features. Spotify sees the track as sad track, which is exact usage of the track in the drama.

## "Love you with all my heart" & "Fallin'"
"Love you with all my heart" and "Fallin'" are used for the same goal and the 2 tracks are also similar in the Spotify features by the similarity in timbre. Both tracks are perceived as negative in the drama. This negative energy is also obsververd by the Spotifty. Resulting positive correlation.
